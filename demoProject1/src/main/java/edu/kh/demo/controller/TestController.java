...
@Controller // 요청, 응답을 제어 할 컨트롤러 역할 명시
			// + Bean으로 등록(== 객체로 생성해서 스프링이 관리)
public class TestController {
		
	// 기존 Servlet : 클래스 단위로 하나의 요청만 처리 가능
	// Spring : 메서드 단위로 요청 처리 가능
	// -> 하나의 클래스로 여러 메서드 처리 가능
	
	// @RequestMapping("요청주소")
	// -> 요청주소를 처리할 메서드를 매핑하는 어노테이션
	// 1) 메서드에 작성 : 요청 주소와 메서드를 매핑
	//					get, post 가리지 않고 매핑(속성을 통해 지정은 가능함)
	
	// 2) 클래스에 작성 : 공통 주소를 매핑
	// -> ex) /todo/Insert, /todo/Select, /todo/Update 가 있을 때,
	
	// @RequestMapping("todo") // /todo 매핑
	// public class 클래스명
	...
	public String testMethod() {
		...
		/* 접두사, 접미사, forward 설정은 View Resolver객체가 담당. */
	}
