...
// 인터셉터가 어던 요청을 가로챌지 설정하는 클래스

@Configuration // 서버가 켜지면 내부 메서드를 모두 수행하라
public class InterceptorConfig implements WebMvcConfigurer{
	
	
	// 인터셉터를 bean으로 등록
	@Bean // 개발자가 만들어서 반환하는 객체를 bean등록 -> 관리는 Spring Container가 수행
	...
	// 동작할 인터셉터 객체를 추가하는 메서드
		...
		public void addInterceptors(InterceptorRegistry registry) {
			...
			// Bean으로 등록 된 BoardNameIntterceptor 객체를 얻어와 인터셉터로 등록
