 ...
 * 
 * [설정 방법]
 *  1. 프로젝명 Application.java 파일 만들어져 있음
 * 	   여기에 @EnableScheduling 어노테이션 추가
 * 
 *  2. 스케쥴러 코드를 작성할 별도 클래스를 생성 후 Bean으로 등록
 *     -> @Component 어노테이션 작성
 *     
 *  3. 해당 클래스에 @Scheduled(시간/주기) 어노테이션을 추가한 메서드 작성
 *  	> 주의 사항
 *  	- 해당 메서드는 반환형이 존재해서는 안됨!!!
 *  	- void 사용하라는 얘기.
 * 
 * 
 * */
...
public class TestScheduling {
	...
	// @Scheduled 어노테이션의 매개변수
	// 1) fixedDelay : 이전 작업이 끝난 후 다음 작업이 실행되기 전까지의 시간을 지정.
	// 2) fixedRate  : 이전 작업이 시작한 후 다음 작업이 실행되기 전까지의 시간을 지정.
	...
	//		/ : 값 증가. 	     ex) 0/5 : 0 부터 시작해서 5씩 증가할 때 마다 수행
	//		? : 특별한 값 없음. (월/요일만 가능)
	//		L : 마지막(월/요일만 가능)
...
//	@Scheduled(cron = "0 0 0 * * *") // 매 자정마다 수행
